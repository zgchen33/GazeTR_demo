...
def fine_tune(subject, data, frame_processor, mon, device, gaze_network, k, steps=1000, lr=1e-4, show=False):
    ...
    '''
    train_gaze_a = torch.FloatTensor(gaze_vector[train_indices, :]).to(device)
    ...
    '''
    valid_gaze_a = torch.FloatTensor(gaze_vector[valid_indices, :]).to(device)
    ...
    # loss = gaze_network.loss()
    optimizer = torch.optim.SGD(
        [p for n, p in gaze_network.named_parameters() if n.startswith('feed')],
        lr=lr,
    )

    gaze_network.eval()
    output_dict = gaze_network(input_dict_valid)
    valid_loss = gaze_network.loss(input_dict_valid, valid_gaze_a).cpu()
    ...
    for i in range(steps):
        ...
        train_loss = gaze_network.loss(input_dict_train, train_gaze_a)
        train_loss.backward()
        optimizer.step()
