...
from normalization import vector_to_pitchyaw
    ...
    def process(self, subject, cap, mon, device, gaze_network, por_available=False, show=False):
        ...
        while ret:
            ...
            if len(face_location) > 0:
                ...
                '''
                processed_patch = np.transpose(patch, [2, 0, 1])
                processed_patch = processed_patch[np.newaxis, :, :, :]
                ...
                if show:
                    ...
                    # output = gaze_network(patch_to_tensor)
                    # input needs [xxx, 3, 224, 224])

                    patch_to_tensor = {'face': torch.tensor(processed_patch, dtype=torch.float).cuda()}
                    output = gaze_network(patch_to_tensor)
                    output = output.data.cpu().numpy()
                    # GazeTR_output: [yaw pitch], however, DTED_output is vector, but it transform to euler,
                    # just like [pitch, yaw]. So we need to make the order of output contrast.
                    output = output[:, ::-1]

                    g_cnn = pitchyaw_to_vector(output.reshape([2, 1]))
                    # print("vector:{}".format(g_cnn))
                    ...
                    cv2.putText(display, '.', (int(x_pixel_hat), int(y_pixel_hat)), font, 0.5, (0, 0, 255), 10,
                                cv2.LINE_AA)
                    # print((int(x_pixel_hat), int(y_pixel_hat)))
                    ...
                    # also show the gaze_arrow:
                    x_center, y_center = (entry['face_bounding_box'][0] + entry['face_bounding_box'][2] / 2), (
                            entry['face_bounding_box'][1] + entry['face_bounding_box'][3] / 2)
                    img_arrow = draw_gaze(img, np.array([x_center, y_center]).reshape((2, 1)),
                                          output.reshape([2, 1]))
